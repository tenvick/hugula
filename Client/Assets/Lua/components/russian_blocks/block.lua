...
local function refreshBlock(data,blockRefs,pos)
	...
	for i = 1,row do
		for j=1,col do
			if(data[i][j]==1)then
				x=(j-1)*tile-offx
				item=blockRefs:Get(indx+1) --refers[indx+1]
...
function Block:on_showed( ... )

	blockManager:gameStart()
end

function Block:on_hide( ... )
	self.enable = false
end

function Block:on_assets_load(items)
