...
local http_ver_hosts = HugulaSetting.httpVerHost ----,"http://192.168.103.200:8055/"} --版本文件加载URL
local ver_url --版本地址
--------------------------------ver url fromat--------------------------
local function set_ver_url()
	local udid = UnityEngine.SystemInfo.deviceUniqueIdentifier --
	if string.match(http_ver_hosts,"%?") then
		ver_url = string.format(http_ver_hosts, APP_VERSION, udid, CUtils.platform, os.time()) -- ResVersion http://gw-warx.dev.tapenjoy.com/api/ver/get?ver=1.0.1&udid=111111&os=ios&time=1491533172
	else
		ver_url = string.format(http_ver_hosts, CUtils.platform, "v"..CODE_VERSION, CUtils.GetRightFileName(VERSION_FILE_NAME))
	end
	print(ver_url)
end
set_ver_url()
...
local function enter_game(manifest)
	local function to_begin(...)
		-- run_times("enter_game begin.lua ")
		if manifest then
...
local function one_file_down(loading_event_arg)
	local m = 1024*1024
	local loaded_s = math.ceil(loading_event_arg.current / m)
	local loaded_t = math.floor(loading_event_arg.total / m)
	set_progress_txt(lua_localization("main_downloading_tips", loaded_s, loaded_t), 4, loading_event_arg.current / loading_event_arg.total)  --string.format("网络资源加载中(消耗流量) %d kb/ %d kb。",loaded_s,loaded_t),4,loaded_s/loaded_t)
end
...
main_update.load_server_file_list = function(load_first) --版本差异化对比
	...
	local function on_server_comp(req)
		...
		local ab =  req.data
		local server_manifest = ab:LoadAllAssets()[1]
		ab:Unload(false)
...
main_update.load_server_verion = function(load_first) --加载服务器版本号
	...
	local function on_comp(req)
		...
		elseif CodeVersion.Subtract(server_ver.version, local_version.version) > 0 then --服务器版本号大于等于当前版本号 --&& server_ver.crc32 ~= local_version.crc32
	...
	begin_fun = function()
		...
		print("start load server version info ... url:" .. ver_url)
		Loader:get_http_data(ver_url, nil, String, on_comp, on_err)--
	end
	begin_fun()
end
...
main_update.compare_local_version = function() --对比本地版本号
	...
	step.on_persistent_error = function(req)
		...
		print("local verion persistent error and the req key is " .. req.key)
		step.compare()
	end
	...
	step.on_streaming_error = function(req)
		...
		if Application.platform == RuntimePlatform.OSXEditor or Application.platform == RuntimePlatform.WindowsEditor then
			...
			ver = string.format(ver, CODE_VERSION)
			local arr = ver:split('\n')
			ver = '"eidtor":true'
	...
	-- print("start load local version info...")
	step.load_streaming()
