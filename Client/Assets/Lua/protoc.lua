...
do
    ...
    function Lexer:constant(opt)
        local c =
            self:full_ident("constant", "opt") or self:number("opt") or self:quote("opt") or self:structure("opt") or
            self:array("opt")
...
do
    ...
    local function check_enum(self, lex, info)
        ...
        for _, v in iter(info, "value") do
            ...
            if not (info.options and info.options.options and info.options.options.allow_alias) then
