...
int do_pb_unpack_full(lua_State* L, const char* data, size_t size, const char* type, char* error)
{
    lpb_State* LS = lpb_lstate(L);
    const pb_Type* t = lpb_type(L,LS, pb_slice(type));
    if (t == NULL) {
        snprintf(error, 256, "type '%s' does not exists", type);
        return 0;
    }
